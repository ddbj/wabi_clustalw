 ...
 */
public class ClustalwRequestValidationUtil extends WabiRequestValidationUtil {
	/*
	...
	static void validateGuidetree(String guidetree, Errors errors) {
		/*
		 * Note: 実装なし。
		 * この guidetree値 はファイルに保存されて、
		 * そのファイル名を CLUSTALWコマンド に与えているだけなので、
		 * OSコマンド・インジェクションのようなセキュリティ問題には
		 * 関係してこない。
		 * 値に不備がある場合は CLUSTALW側 でエラーにする。
	...
	static void validateDnaMatrix(String dnaMatrix, Errors errors) {
		/*
		 * Note: 実装なし。
		 * この dnaMatrix値 はファイルに保存されて、
		 * そのファイル名を CLUSTALWコマンド に与えているだけなので、
		 * OSコマンド・インジェクションのようなセキュリティ問題には
		 * 関係してこない。
		 * 値に不備がある場合は CLUSTALW側 でエラーにする。
	...
	/**
	 * Clustalw入力データ の parameters値 の先頭が受け付け可能なオプションの場合は、
	...
	private static boolean removeAcceptOptions(Deque<String> parameters, String acceptOptions, int recursiveCounter) {
		...
		if (firstToken.indexOf("=") >= 0) {
			String[] key_value = firstToken.split("=");
			if (key_value.length != 2) {
				return false;
			}
			//オプション名のチェック
			if (acceptOptions.indexOf(key_value[0].substring(1))<0) {
				return false;
			}
			//オプション値のチェック
			if (!patternParameters_each.matcher(key_value[1]).matches()) {
				return false;
			}
		...
		//-optionの場合
			...
			//オプション名のチェック
			if (acceptOptions.indexOf(firstToken.substring(1))<0) {
				return false;
			}
			parameters.removeFirst();
		}
