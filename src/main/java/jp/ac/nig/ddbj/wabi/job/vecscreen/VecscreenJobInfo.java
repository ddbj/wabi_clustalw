package jp.ac.nig.ddbj.wabi.job.vecscreen;
...
import jp.ac.nig.ddbj.wabi.controller.vecscreen.VecscreenController;
...
public class VecscreenJobInfo extends WabiJobInfo {
...
//	private final static Object lock = new Object();
	 ...
	 */
	public VecscreenJobInfo(String requestId) throws IOException {
		super(requestId);
	}
	...
	public boolean existsFinishedFile() {
		String path = getWorkingDir() + VecscreenController.FINISHED_FILE;
		return new File(path).exists();
	}

	public boolean existsOutFile() {
		String path = getWorkingDir() + VecscreenController.WABI_OUT_FILE;
		return new File(path).exists();
	}
	...
	public String readJobId() throws IOException {
		String path = getWorkingDir() + VecscreenController.UGE_JOB_ID_FILE;
	...
	public String reGenerateRequestId() throws IOException {
		...
		requestId = VecscreenController.outfilePrefix + ArrayUtil.join("-", new String[]{year, month+day, hour+min, sec, millisec, randomSuffix}).trim();
	...
	public boolean existsUserRequestFile() {
		String path = getWorkingDir() + VecscreenController.USER_REQUEST_FILE;
		return new File(path).exists();
	}
